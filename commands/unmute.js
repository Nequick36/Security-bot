...
module.exports.run = async (bot, message, args) => {
 let role = message.guild.roles.find(r => r.name === 'Extreme | Mute') 
  ...
  if(!user) return message.channel.send({embed:{description:`**:x: MORATE TAGATI MEMBERA KOJEG ZELITE UNMUTATI :x:**`, color:0xff0000}})
 ...
 if(!message.member.hasPermission("MANAGE_MESSAGES")) return message.channel.send({embed:{description:`**:x: VI NEMATE DOZVOLU DA KORISTITE OVU KOMANDU :x:**`, color:0xff0000}})
  ...
  message.channel.send({embed:{description:`:white_check_mark: ${user} je unmutovan! od strane Osoblja: ${message.author.username} :white_check_mark:`, color:0xff0000}})
}

...
exports.run = async (bot, message) => {
  ...
  fs.appendFile('events/chatlogs.txt', `[${moment(date).format('DD.MM.YYYY')}][${moment(date).format('hh:mm:ss')}][${message.channel.name}] ${message.member.username || message.author.username}#${message.author.tag.split('#').slice(1).join(" ")}: ${message.content}\n`, (err) => {
    if(err) console.log(err)
  })
